...
function updatePlayer(player, ctx) {
    ...
    if (keys['s'] && player.y+player.frames[player.currFrame][3]<ctx.canvas.height-(16+player.spd)) {
        player.y += player.spd;
    }
    if (keys['a'] && player.x>16+player.spd) {
        player.x -= player.spd;
    }
    if (keys['d'] && player.x+player.frames[player.currFrame][2]<ctx.canvas.width-(16+player.spd)) {
        player.x += player.spd;
    }
